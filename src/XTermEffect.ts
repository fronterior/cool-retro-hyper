...
type XTermEffectParameters = {}

export class XTermEffect {
  static THREE = THREE
   ...
   */
  constructor(options: XTermEffectParameters) {
    ...
    this.clock = new XTermEffect.THREE.Clock({ autoStart: false })
   ...
   */
  prep() {
    const { Scene, WebGLRenderer, OrthographicCamera } = XTermEffect.THREE
    const { EffectComposer, RenderPass } = XTermEffect.POSTPROCESSING
  ...
  setPasses(passes) {
    const { Pass, EffectPass } = XTermEffect.POSTPROCESSING
  ...
  _createMeshes(numLayers) {
    const { PlaneGeometry, MeshBasicMaterial, Mesh } = XTermEffect.THREE
  ...
  addTextures(xTermLayers) {
    ...
    const { CanvasTexture, LinearFilter } = XTermEffect.THREE

    if (xTermLayers.length !== this.scene.children.length) {
      console.warn(
        'hyper-postprocessing error: number of meshes does not equal number of render layers...',
      )
    }
  ...
  readDimensions() {
    ...
    this._setUniforms({
      aspect: w / h,
      resolution: new XTermEffect.THREE.Vector2(w * dpRatio, h * dpRatio),
    })
  }
  ...
  startAnimationLoop() {
    ...
    const xTermMaterials = Array.from(this._xTermLayerMap.values()).map(
      (m) => m.material,
    )
    const timeUniforms = this._shaderPasses
      .filter((pass) => {
